...
void VL53L0X_i2c_init(void)
{
	...
	GPIO_Initure.Pin=GPIO_PIN_11|GPIO_PIN_12; //PC11、PC12
	GPIO_Initure.Mode=GPIO_MODE_OUTPUT_PP;  	//推挽输出
	GPIO_Initure.Pull=GPIO_PULLUP;        	  //上拉
	GPIO_Initure.Speed=GPIO_SPEED_HIGH;   	 	//高速

	HAL_GPIO_WritePin(GPIOC,GPIO_PIN_11|GPIO_PIN_12,GPIO_PIN_SET);	//PC11,PC12 输出高	
...
u8 VL_IIC_Wait_Ack(void)
{
	u8 ucErrTime=0;
	VL_SDA_IN();  //SDA设置为输入  
	VL_IIC_SDA=1;Delay_us_Z(1);	   
	VL_IIC_SCL=1;Delay_us_Z(1);	 
	while(VL_READ_SDA)
	{
		ucErrTime++;
		if(ucErrTime>250)
