...
uint32_t tim6_time1 = 0;
uint32_t tim6_time2 = 0;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)    //定时器中断回调
{
		...
		else if(htim ->Instance == TIM5)		//50ms
		{
				bat_adc_val = Filtering2_Median_Average_Z(adc_raw_buf[0],1);
			
				if(HAL_GPIO_ReadPin(KEY_GPIO_Port,KEY_Pin) == GPIO_PIN_RESET)		key_scan_count++;
		}
